...
public class UpcomingTalkDao {
	...
	private PreparedStatement pstSelect, pstInsert, pstUpdate, pstSelectById,
			pstDelete;
	...
	public UpcomingTalkDao() throws Exception {
		...
		pstSelectById = cn
				.prepareStatement("select u.talkid,u.title,u.presenter,u.venue,u.date,u.time,u.description from Upcoming_Talks u,Audiences a,techusers t "
						+ "where a.emailid=t.emailid and u.talkid=a.talkid and u.date>= curdate() and t.emailid = ? "
						+ "order by u.date");
		pstDelete = cn
				.prepareStatement("Delete from Upcoming_Talks WHERE talkid=?");
		System.out.println("dao created...");
		// pst3 =
		// cn.prepareStatement("Select user_id from user where user_name = ?");
	}
	...
	public boolean setNewTechTalk(UpcomingTalk upcomingTalk)
			throws SQLException {
		int status;
	...
	public List<UpcomingTalk> getAllUpcomingTalks() throws SQLException {
		...
		try (ResultSet resultSet = pstSelect.executeQuery()) {
			while (resultSet.next())
				talkList.add(new UpcomingTalk(resultSet.getInt(1), resultSet
						.getString(2), resultSet.getString(3), resultSet
						.getString(4), resultSet.getDate(5), resultSet
						.getTime(6), resultSet.getString(7)));
	...
	public boolean updateTalkById(UpcomingTalk upcomingTalk, int talkId)
			throws SQLException {
		int status;
	...
	public List<UpcomingTalk> getUpcomingTalksByEmailId(String email)
			throws SQLException {
		List<UpcomingTalk> talkList = new ArrayList<>();
